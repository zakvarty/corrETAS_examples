...
#   mu_prior = c(0.1, 0.1)
# )
# saveRDS(object = mcmc_1, file = "./output/mcmc/mcmc_1.RDS")
mcmc_1 <- readRDS("./output/mcmc/mcmc_1.RDS")
...
xrange = 1:5000
pdf(
  file = "./output/mcmc/trace_plots/trace_plots_1.pdf",
  width = 7,
  height = 5)
opar <- par()
...
#   mu_prior = c(0.1, 0.1)
# )
# saveRDS(object = mcmc_2, file = "./output/mcmc/mcmc_2.RDS")
mcmc_2 <- readRDS("./output/mcmc/mcmc_2.RDS")


# parameter trace plots --------------------------------------------------------
pdf(
  file = "./output/mcmc/trace_plots/trace_plots_2.pdf",
  width = 7,
  height = 5)
opar <- par()
par(mar = c(4.1,4.1,2.1,1.1))
...
#   mu_prior = c(0.1, 0.1)
# )
# saveRDS(object = mcmc_3, file = "./output/mcmc/mcmc_3.RDS")
mcmc_3 <- readRDS("./output/mcmc/mcmc_3.RDS")
...
## 04: Posterior samples 4 - Falsely assume single magnitude distribtion -------
# set.seed(2212)
# branching_init <- (seq_along(cat$b) - 1) * rbinom(length(cat$b), size = 1, prob = 0.4)
...
#   mag_sds = rep(0.01,2),
#   mu_prior = c(0.1,0.1))
# saveRDS(object = mcmc_4, file = "./output/mcmc/mcmc_4.RDS")
mcmc_4 <- readRDS("./output/mcmc/mcmc_4.RDS")

# parameter trace plots --------------------------------------------------------
pdf(
  file = "./output/mcmc/trace_plots/trace_plots_4.pdf",
  width = 7,
  height = 5)
opar <- par()
par(mar = c(4.1,4.1,2.1,1.1))
...
# Parameter recovery: magnitude posteriors

pdf(file = "./output/parameter_recovery/magnitude_parameter_posteriors.pdf", width = 4, height = 4)
...
lines(density(mcmc_3$mag$nu_m0,  from = x_low, to = x_high),
      col = "black",
      lwd = 2)
abline(v = mag_values[1], lty = 2 , lwd = 2, col = "black")
abline(v = mag_values[3], lty = 2 , lwd = 2, col = "red")
...
lines(density(mcmc_3$mag$xi_m0,  from = x_low, to = x_high),
      col = "black",
      lwd = 2)
abline(v = mag_values[2], lty = 2 , lwd = 2, col = "black")
abline(v = mag_values[2], lty = 3 , lwd = 2, col = "red")
contour(
  x = MASS::kde2d(x = mcmc_3$mag$nu_m0, y = mcmc_3$mag$xi_m0),
  levels = seq(0,350, by = 25),
  xlim = c(0.0,0.7),
  ylim = c(-0.3,0.3),
  col = "black",
  xlab = expression(nu[m]),
...
points(x = mag_values[c(1,3)], y =  mag_values[c(2,4)], pch  =16, cex = 1.5, col = "blue")
dev.off()
...
pdf("./output/parameter_recovery/dual_mag_return_level_plots.pdf", width = 4, height = 4)
...
for(i in seq_along(return_periods)){
  dual_return_levels[,i] <- mcmc_return_levels(mcmc_3, return_periods[i], m_0_value)
...
pdf("./output/parameter_recovery/mag_return_level_plots_dual_vs_single.pdf",
    width = 4, height =  4)
...
prop_background_samples_3 <- rowMeans(mcmc_3$b >0)
prop_background_samples_4 <- rowMeans(mcmc_4$b >0)
pdf("./output/parameter_recovery/B_posterior_proportion_triggered_dual_vs_single.pdf",
    width = 7,
    height = 5)
par(mar = c(4.5,4.5,2.1,2.1))
plot(density(prop_background_samples_3,from = 0.1, to = 0.25),
     col = "blue",
     lwd = 3,
     xlim = c(0.18,0.22),
     ...
     main ="",
     cex.lab = 1.25)
lines(density(prop_background_samples_4), col = "orange", lwd = 3)
abline(v = mean(cat$b >0), col = 1, lty = 2, lwd = 3)
dev.off()

sqrt(mean((prop_background_samples_3 - n_trig / length(cat$ts))^2))
sqrt(mean((prop_background_samples_4 - n_trig / length(cat$ts))^2))
...
pdf("./output/parameter_recovery/B_posterior_proportion_correct_dual_vs_single.pdf",
    width = 7,
    height = 5)
par(mar = c(4.5,4.5,2.1,2.1))
plot(density(prop_correct_samples_3,from = 0.9, to = 1),
     col = "blue",
     lwd = 3,
     xlim = c(0.9,1.0),
     ...
     main ="",
     cex.lab = 1.25)
lines(density(prop_correct_samples_4), col = "orange", lwd = 3)
abline(v = 1, col = 1, lty = 2, lwd = 3)
dev.off()

sqrt(mean((prop_correct_samples_3 - 1)^2))
sqrt(mean((prop_correct_samples_4 - 1)^2))
...
pdf("./output/parameter_recovery/B_elementwise_change_in_posterior_proportion_correct_dual_vs_single.pdf",
    width = 7,
    height = 5)
...
pdf(file = "./output/parameter_recovery/ETAS_parameter_recovery_dual_vs_single.pdf",
    width = 4, height = 4)
...
plot(d_3, lwd = 2, xlab = "C", ylab = "Posterior density", col = "blue", main ='')
lines(d_4, col = "darkorange", lwd = 2)
abline(v = etas_values[2], lwd = 2, lty = 2)

d_3 <- density(mcmc_3$etas$a, from = 0, to = 0.6)
d_4 <- density(mcmc_4$etas$a, from =  0, to =  0.6)
...
plot(d_3, lwd = 2, xlab = expression(xi[t]), ylab = "Posterior density", col = "blue", main ='')
lines(d_4, col = "darkorange", lwd = 2)
abline(v = etas_values[4], lwd = 2, lty = 2)
